...
skip_input_url_ls = ['https://woxer.com/products/gift-card']
skip_output_url_ls = []
...
class Pc(Get_Product):
    ...
    def zdy_zd(self, url):
        ...
        dic = {}
        for node in html.xpath('//div[@id="productAccordion"]/div'):
            name = node.xpath('./div/h3/text()')[0]

            for i in ['Details', 'Size & Fit','Materials & Care']:
                if i in name:
                    value = node.xpath('./div')[1]
                    dic[i] = Tool.HTML.clean_product_desc(value)
                    break
            else:
                for i in ['Shipping & Returns']:
                    if i in name:
                        break
                else:
                    print(f'未知字段:{name}')
