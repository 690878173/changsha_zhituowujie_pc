...
class IMG_download_ljp:
    ...
    def get_image_urls(self):
        ...
        default_filename = f"{script_dir}_quchong.csv"
        safe_print(f'默认使用文件:{default_filename}')
        input_csv = self.config['PATHS'].get('input_csv',default_filename)
        if not input_csv:
            input_csv = default_filename
            ...
            try:
                ...
                if self.is_valid_image_response(response, content):
                    ...
                    if self.log_each_success:
                        safe_print(f"已下载: {filename},url:{image_url}")
                    return None
