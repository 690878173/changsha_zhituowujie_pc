...
from base_f import Tool, zs, product_zdy_zd_name
...
hc_path = Tool.File.path_add_site('hc/product_data.json')
...
def get_zd(url):
    ...
    try:
        try:
            product_details = html.xpath('//div[@class="pss-card"]')[0]
        except Exception as e:
            return None

        product_details = etree.tostring(product_details, encoding="unicode")
        return {'PRODUCT DETAIls': product_details}
    except Exception as e:
        print(f"获取自定义字段失败,url:{url}: {e}")
    return None
...
def convert_to_woocommerce_format(shopify_product, custom_categories=None):
    ...
    # 添加自定义字段到父类
    zdy_data = shopify_product.get(product_zdy_zd_name, {})
    if zdy_data:
...
def create_variation_products(shopify_product, parent_product):
    ...
    for variant in shopify_product.get('variants', []):
        ...
        # 变体不需要自定义字段值，置空
        zdy_data = parent_product.get(product_zdy_zd_name, {})
        if zdy_data:
            for key in zdy_data:
                variation[key] = ''
        custom_meta_keys = [k for k in parent_product.keys() if "(product.c_f." in k]
        for k in custom_meta_keys:
            variation[k] = ""
...
def get_product_by_handle(session, handle, retries=5):
    ...
    for attempt in range(retries):
        try:
            ...
            shopify_product[product_zdy_zd_name] = zdy_data
