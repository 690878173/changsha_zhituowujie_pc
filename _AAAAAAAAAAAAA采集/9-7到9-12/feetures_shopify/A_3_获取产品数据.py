...
class Pc(Get_Product):
    ...
    def zdy_zd(self, url):
        ...
        divs = html.xpath('//div[@class="product__accordion accordion quick-add-hidden"]/details')

        dic = {}
        for div in divs:
            name = div.xpath('./summary//text()')
            name = ''.join(name).strip()

            for i in ['DETAILS + TECH',' MATERIALS & CARE']:
                if i in name:
                    value = div.xpath('./div')[0]
                    name = i.replace('+','and').replace('&','and')
