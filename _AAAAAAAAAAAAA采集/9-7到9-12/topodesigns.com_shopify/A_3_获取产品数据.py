...
class Pc(Get_Product):
    ...
    def zdy_zd(self, url):
        ...
        dic = {}
        divs = html.xpath('//div[@class="product__text rte quick-add-hidden details-metaobject-custom"]')


        h2 = html.xpath('//div[@class="product__text rte quick-add-hidden details-metaobject-custom"]/h2/text()')[0]

        for i in ['Features & Specs']:
            if i in h2:
                dic[i] = Tool.HTML.clean_product_desc(divs[0])
        for node in html.xpath('//div[@class="product__accordion accordion quick-add-hidden details-metaobject-custom"]/details'):
            name = node.xpath('./summary/div/h2/text()')[0]

            for i in ['Sustainability', 'Description']:
                if i in name:
