...
class Pc(Get_Product):
    ...
    def zdy_zd(self, url):
        ...
        h5_ls = html.xpath('//div[@id="accordion_tab"]/h5')
        value_ls = html.xpath('//div[@id="accordion_tab"]/div')
        for index,node in enumerate(h5_ls):

            name = node.xpath('./text()')[0]

            for i in ['Design Details', 'Specification']:
                if i in name:
