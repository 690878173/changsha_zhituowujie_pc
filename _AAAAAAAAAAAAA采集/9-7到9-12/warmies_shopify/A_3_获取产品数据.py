...
class Pc(Get_Product):
    ...
    def zdy_zd(self, url):
        ...
        dic = {}
        tabs = html.xpath('//x-tabs')

        if tabs:
            tabs = tabs[0]
            button = tabs.xpath('./button/text()')
            values = tabs.xpath('./div')


            for index, value in enumerate(button):

                for i in ['Care','Description']:
                    if i in value:
                        dic[i] = Tool.HTML.clean_product_desc(values[index])
                        break
