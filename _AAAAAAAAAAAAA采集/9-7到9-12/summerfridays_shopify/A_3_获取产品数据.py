...
class Pc(Get_Product):
    ...
    def zdy_zd(self, url):
        ...
        dic = {}
        for node in html.xpath('//div[@class="media-accordions__accordions-inner"]'):
            name = node.xpath('./div/h2/text()')
            ...
            for i in ['Key Ingredients', 'This Set Includes']:
                if i in name:
                    value = node.xpath('./ul')
                    v_str = ''.join(Tool.HTML.clean_product_desc(i) for i in value)

                    value = f'<div>{v_str}</div>'
                    dic[i] = value
                    break
