...
class Pc(Get_Product):
    ...
    def zdy_zd(self, url):
        ...
        dic = {}
        divs = html.xpath('//div[@data-orientation="vertical"]/div[@data-replo-collapsible and @data-orientation="vertical"]')
        for div in divs:
            name = div.xpath('./div[1]/div/div/span/p/text()')
            name = ''.join(name).strip()

            for i in ['Features','materials','washing & care']:
                if i in name:
                    value = div.xpath('./div[2]')[0]
