...
class Pc(Get_Product):
    ...
    def zdy_zd(self, url):
        ...
        details = html.xpath('//div[@id="shopify-section-template--19410977783984__details-panel"]//div[@class="details-panel--content"]')

        ingred = html.xpath('//div[@id="shopify-section-template--19410977783984__ingredients-panel"]//div[@class="details-panel--content"]')
        if details:
            dic['Detailas'] = Tool.HTML.clean_product_desc(details[0])

        if ingred:
            dic['Ingredientes'] = Tool.HTML.clean_product_desc(ingred[0])
