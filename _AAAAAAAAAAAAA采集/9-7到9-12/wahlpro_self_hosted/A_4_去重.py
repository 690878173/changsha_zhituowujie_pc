...
def merge_categories(group):
    ...
    Wahl 有 5 个 SKU 同时存在“独立简单商品页”和“可配置商品的子变体”两种
    形态（例如 08148 既是单页也是 Magic Clip 的 Burgundy 变体）。同一 SKU
    只应导出一条，这里优先保留变体行，让变体族完整，分类照旧全部并入。
    """
    # 获取第一个非空值作为基础
    ...
    variation_rows = group[group['Type'] == 'variation']
    first_row = variation_rows.iloc[0:1] if len(variation_rows) else group.iloc[0:1]
