...
EXTRA_META_COLUMNS = [
    ...
    "What's in the Box(product.metafields.c_f.what's_in_the_box)",
...
def split_images(value):
    """Tool.Product 用配置里的分隔符拼接图片，这里按同一规则拆回来。"""
    text = str(value or '')
    separator = Tool.config.images_split or ','
...
def generate_shopify_rows(df):
    ...
    for _, parent in parent_products.iterrows():
        ...
        all_images = split_images(parent.get('Images', ''))
    ...
    for _, product in simple_products.iterrows():
        ...
        all_images = split_images(product.get('Images', ''))
        if all_images:
