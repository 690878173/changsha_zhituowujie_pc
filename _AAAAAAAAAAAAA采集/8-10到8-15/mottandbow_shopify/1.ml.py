...
def f1(url_dic):
    ...
    ml1 = html.xpath('//nav[@class="header__inline-menu"]/ul/li/details-disclosure/details')
    header_node = ml1


    for node in header_node:

        c_node = node.xpath('./summary')[0]
        _name = Tool.HTML.get_text(c_node)
        _url = c_node.get('data-href')
        _url = Tool.URL.add_site(_url).replace('/en-jp','')

        if 'About us' in _name:
            continue

        url_dic[_name] = {'url':_url,'child':{}}

        childs = node.xpath('./div/ul/li')
...
def f2(dic, childs):
    for child in childs:
        ...
        c_node = child.xpath('./div/div/div')[0]
