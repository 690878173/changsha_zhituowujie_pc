...
def get_zd(url):
    try:
        ...
        print(url)
        ds = html.xpath('//description-tab')

        d_name_dic = {
            'details': 'Fit-Details',
            'fit': 'Fit-Details',
            'fit-details': 'Fit-Details',
            'material-and-care': 'Material-and-Care',
            'care': 'Material-and-Care',

        }

        no_ls = ['returns-tab', 'description', 'product-features','Description','sheer level','shipping and returns','Shipping and Returns','quality-and-care']

        def _check_(name):
            name = name.lower().replace("'",'').replace('"','')
            if d_name_dic.get(name):
                name = d_name_dic[name]

            if 'product features' in name:
        ...
        def _m1():
            ...
            tx = html.xpath('//description-tab/div[@class="description-title"]/div[@class="description-content"]')[0]
            name_ls = tx.xpath('./h2/text()')
            df_ls = tx.xpath('./div')
            if len(name_ls) != len(df_ls):
                Tool.print(f'name和df数量不一致:{url}')
                raise
            for i in range(len(name_ls)):
                name = name_ls[i]
                if name in no_ls:
                    continue
                if name.lower() in no_ls:
                    continue
                if name.replace('-',' ') in no_ls:
                    continue
                df = df_ls[i]
                df = Tool.HTML.clean_product_desc(df)
        ...
        def _(_ds):
            ...
            for _d in _ds:
                _d_name = _d.xpath('./h2')[0]
                _d_name = _d_name.get('id')

                if _d_name in no_ls:
                    continue
                _d_df = _d.xpath('./div[@class="description-content"]')[0]
                _d_df = Tool.HTML.clean_product_desc(_d_df)


                name = _check_(_d_name)
                if dic.get(name):
                    dic[name] += _d_df
                else:
                    dic[name] = _d_df
        ...
        def _m2():
            ds = html.xpath('//alt-description-tab')[0].xpath('./div')

            _(ds)

        def _m3():
            ds = html.xpath('//description-tab')[0].xpath('./div')
            _(ds)

        functions = [_m1, _m2, _m3]  # 按尝试顺序排列
        last_exception = None
...
def get_product_by_handle(session, handle, retries=5):
    ...
    for attempt in range(retries):
        try:
            ...
            tqdm.write(f"\n[WARN] {url} 网络异常: {e}, 第 {attempt + 1}/{retries} 次重试")
...
def main():
    ...
    for url in tqdm(product_urls):
        ...
        if 'navidium-shipping-protection-2' in url:
            continue
    ...
    for category_path, urls in detail_url_data.items():
        for url in urls:
            ...
            if handle == 'navidium-shipping-protection-2':
                continue
