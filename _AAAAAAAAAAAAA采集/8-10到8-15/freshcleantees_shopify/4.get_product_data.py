...
def get_zd(url):
    ...
    #
    ds = html.xpath('//collapsible-row[@class="product__accordion accordion"]/details')
    for d in ds:
        tx = d.xpath('./summary//text()')
        tx = ''.join(tx).strip()
        if '100-Day Satisfaction Guarantee' in tx:
            continue
        elif 'Recurring Subscription' in tx:
            continue

        elif 'Final Sale' in tx:
            continue
        elif 'Fabric' in tx:
            tx = 'Fabric Care'
        elif 'Fit' in tx:
        ...
        dt = d.xpath('./div')[0]
        dt = Tool.HTML.clean_product_desc(dt)
        dic[tx] = dt
    print(dic)
    return dic
