...
def get_zd(url):
    ...
    for d in ds:
        ...
        d_class_dic = {
            'tab-the-details' : 'Fabric and Details',
            'tab-the-fit' :'The Fit'
        }

        if 'tab-description' in d_class:
            continue
        elif 'product-tabs__pairs-with' in d_class:
            continue

        for k in d_class_dic.keys():
            if k in d_name:
                d_name = k
                break
        # print(d_name,d_class)



        d_df = d.xpath('./div[@class="product-tab__content-container"]/div')[0]


        d_df = Tool.HTML.clean_product_desc(d_df)
        # if d_name == 'The Fit':
        #     print(f'The Fit:{d_df}')
        dic[d_name] = d_df
