...
# 排除某些 URL
input_url_no_ls = ['https://www.yankeecandle.com/giftcards','https://www.yankeecandle.com/personalized-candles.html','https://www.yankeecandle.com/yc-collection.html']
output_url_no_ls = []
...
def get_detail_url(url, par):
    ...
    product_url = html.xpath(
        '//div[@class="css-1h6psof"]//a[@class="chakra-link css-1no4gh1"]/@href')
    detail_links = [Tool.URL.add_site(i) for i in product_url]


    next_kink = html.xpath('//*[@id="app-main"]/div[2]/div[5]/div/div[2]/div[1]/a[2]/@href')[0]
    next_link = Tool.URL.add_site(next_kink)
...
def get_detail_all_url(url):
    ...
    while True:
        ...
        if _num >20:
            ...
            Tool.print(f'url:{url}:正在处理:第{_num}页')
    return ls
...
def main():
    ...
    for name, url in urls_data.items():
        ...
        if isinstance(ts_num, int) and num >= ts_num:
            Tool.print(f"已处理 {num} 条数据，已超出限制，任务结束")
            break
        num += 1
