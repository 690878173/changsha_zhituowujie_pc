...
def get_zd(url):
    ...
    print(url)
    ds = html.xpath('//div[@class="md:w-1/2 p-4"]/div')
    for d in ds:
        d_name = d.xpath('.//div[@class="font-serif flex-grow text-xl"]//text()')

        d_name = ''.join(d_name).strip()
        if 'Product Information' in d_name:
            continue
        if 'How to Use' in d_name:
            continue

        if 'FAQs' in d_name:
            continue

        if 'Amino Acids' in d_name:
            continue


        script_node = d.xpath('.//script//text()')

        tx = ''.join(script_node)
        input_match = re.search(r'const input = `([\s\S]*?)`;', tx)
        if not input_match:
            continue

        raw_html = input_match.group(1)  # 此时仍包含 &lt; &amp; 等实体

        # 解码 HTML 实体（将 &lt; 转为 <，&amp; 转为 & 等）
        import html
        decoded_html = html.unescape(raw_html)

        d_df = decoded_html

        d_df = Tool.HTML.clean_product_desc_str(d_df)
