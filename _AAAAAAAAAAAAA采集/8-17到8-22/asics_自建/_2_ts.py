...
catch_path = Tool.File.path_add_site('hc/2/_ts_data.json')

index_path = Tool.File.path_add_site('hc/2/_ts_index.json')
...
class Sc(object):
    ...
    def get_detail_url(self, url, par: dict):
        ...
        product_urls = []
        try:
            e_page.goto(url,timeout=20000)


            e_page.wait_for_selector(".productTile__root", timeout=10000)

            time.sleep(0.5)
            link_elements = e_page.locator("xpath=//div[contains(@class, 'productTile__root')]/a").all()
            urls = [el.get_attribute("href") for el in link_elements]

            tot = e_page.locator('xpath=//span[@data-test="total-hit-count"]').inner_text()
            tot = int(tot.replace(',', '').replace('"', '').replace('(', '').replace(')', ''))

            ls = [Tool.URL.add_site(i) for i in urls]
            if tot < 24 * int(page):
                is_next = False
                par['next_url'] = None
                ...
                is_next = True
                par['next_url'] = raw_url + f'?page={int(page)+1}'

            product_urls = ls
        except Exception as e:
            print(f"访问 {url} 超时（可能重定向循环），跳过:{e}")
