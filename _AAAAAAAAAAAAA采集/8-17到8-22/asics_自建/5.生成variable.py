...
def merge_comma_values(values):
    """Merge comma-separated values while retaining their first-seen order."""
    result = []
...
def process_csv(merge_fields=None, desc_like_fields=None):
    ...
    # Parent is the ASICS style/product number (for example ANA_1011C235).
    # Categories describe where the SKU was found; they must not split one
    # style into several Shopify variable products.
    group_fields = ["Parent"]
    ...
    for group_key, group in variation_group:
        ...
        if "Categories" in group.columns:
            variable_row["Categories"] = merge_comma_values(group["Categories"])
