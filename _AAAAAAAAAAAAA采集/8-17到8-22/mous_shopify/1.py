...
def f1(url_dic):
    ...
    ml1 = html.xpath('//div[@class="top-level-links-wrapper"]/div[@class="top-level-links"]/div[@class="link-group"]')
    header_node = ml1


    for node in header_node:

        div1 = node.xpath('./div[@class="top-level-link"]')[0]

        _url = div1.get('data-href')
        _name = div1.xpath('./text()')[0].strip()
        _url = Tool.URL.add_site(_url)

        url_dic[_name] = {'url':_url,'child':{}}

        childs = node.xpath('./div[@class="mega-menu-dropdown_outer-wrapper"]/div[@class="mega-menu-dropdown"]//div[@class="mega-menu-column"]')
...
def f2(dic, childs):
    for child in childs:
        ...
        n_childs_ls = child.xpath('./div[@class="mega-menu-group"]/ul/li')
