...
class Pc(Get_Product):
    ...
    def zdy_zd(self,url):
        ...
        dsc = html.xpath('//div[@class="product__accordion"]')

        try:
            for ds in dsc:
                title = ds.xpath('./button/text()')[0].strip()
                if 'Features' in title:
                    content = ds.xpath('./div')[0]
                    content = Tool.HTML.clean_product_desc(content)
                    return {'Features':content}
        except Exception as e:
            Tool.print(f'自定义字段获取失败:{url},e:{e}')
...
if __name__ == '__main__':
    pc = Pc(
        tool=Tool,input_file=input_file, output_file=output_file, fail_file=None,
        catch_path=catch_path, index_path=index_path, output_ts_file=output_ts_file,ts_num=ts_num,
        catch_save_num = catch_save_num,
        skip_input_url_ls=skip_input_url_ls, fieldnames=fieldnames, max_threads=10,if_wp=if_wp
    )
