...
def check_url(url):
    # 由具体站点覆盖,默认原路返回
    for i in ['/products/','https://support.turtleboxaudio.com/support/home','https://corporate.turtleboxaudio.com/']:
        if i in url:
            return 'skip',None
    if url in [Tool.URL.base_url]:
        return 'no_url',None

    for i in ['/pages/','/blogs/']:
        if i in url:
            return 'no_url',None
    return 'ok',url
...
def f1(url_dic):
    ...
    ml1 = html.xpath('//nav[@data-header-nav]/ul/li')
    header_node = ml1

    print(len(header_node))
    for node in header_node:

        c_node = node.xpath('./div/a')
...
def run():
    ...
    url_dic['Gameday'] = {'url': 'https://turtleboxaudio.com/collections/gameday-series', 'child': {}}
    url_dic['corporate'] = {'url':'https://corporate.turtleboxaudio.com'}
