...
class Pc(Get_Product):
    ...
    def zdy_zd(self, url):
        ...
        fields = {}
        # short_desc = html.xpath('string((//*[contains(@class, "PDP-Product-Short-Description")])[1])')
        # if short_desc := ' '.join(short_desc.split()):
        #     fields['Short Description'] = short_desc

        feature_sections = html.xpath('//section[h2[normalize-space()="Features"]]')
        ...
        for child in feature_section:
            ...
            if child.tag == 'ul' and child.xpath('./li[h3]'):
                for item in child.xpath('./li[h3]'):
                    ...
                    if label and detail:
                        if label in ['Material','MATERIAL']:
                            label = 'Materials'
                        if label in ['Features','Materials','Fit']:
                            fields[label] = detail
                continue
