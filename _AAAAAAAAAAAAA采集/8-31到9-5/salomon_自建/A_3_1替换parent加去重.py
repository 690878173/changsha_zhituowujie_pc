...
df = pd.read_csv(Tool.File.path_add_site('hc/3/result.csv'))
...
# 在 DataFrame 上应用
df['Parent'] = df.apply(
    lambda row: generate_unique_id(str(row['Parent']), str(row['url'])),
    axis=1
)


df.to_csv(Tool.File.path_add_site('hc/3/new.csv'), index=False)


import pandas as pd
from config import Tool

input_file = Tool.File.path_add_site(r'hc/3/new.csv')
