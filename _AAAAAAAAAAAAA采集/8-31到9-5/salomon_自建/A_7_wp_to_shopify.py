...
# ================= 维护区：自定义字段写在这里 =================
EXTRA_META_COLUMNS = [
    'Features & Fabrics(product.metafields.c_f.features_fabrics)',
    'More details(product.metafields.c_f.more_details)'
]

input_csv = Tool.File.path_add_site(r"fwq/variable.csv")
...
def generate_shopify_rows(df):
    ...
    for _, parent in parent_products.iterrows():
        ...
        all_images = [img.strip() for img in str(parent.get('Images', '')).split('l__|__p') if img.strip()]
