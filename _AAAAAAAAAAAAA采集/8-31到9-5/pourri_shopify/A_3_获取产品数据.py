...
class Pc(Get_Product):
    ...
    def zdy_zd(self, url):
        ...
        dic = {}
        lis = html.xpath('//div[@class="tabs__info--tabs"]/div')
        for li in lis:
            name = li.xpath('./h3/text()')[0]

            for i in ['How it Works','What’s Funk Lock Technology','Safety + Warnings','Scent Notes']:
                if i in name:
    ...
    def fetch_product(self, url, category) -> list:
        ...
        def _f(x):
            if pd.isnull(x):
                return x
            return x.replace('20% Off!')
        woo_product['Description'] = woo_product['Description'].apply(_f)
