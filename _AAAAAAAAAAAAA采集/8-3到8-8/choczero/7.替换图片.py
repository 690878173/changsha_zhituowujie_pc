...
import os
import hashlib
from config import images_split,Tool

csv_input_path = Tool.File.path_add_site(r'data/quchong.csv')
failed_images_path = Tool.File.path_add_site(r'data/failed_images.txt')
csv_output_path = Tool.File.path_add_site(r'data/picture.csv')

download_img_path = Tool.File.path_add_site(r'fwq/quchong.csv')
...
def run():
    ...
    fwq_df = df.copy()
    # 3. 处理图片URL
    df['Images'] = df['Images'].apply(lambda x: generate_new_image_url(x, failed_images, web_name))

    fwq_df['NewImages'] = df['Images']
    Tool.File.save_csv(fwq_df[['Type', 'SKU', 'Parent','NewImages','Images']], download_img_path)
...
def generate_new_image_url(image_urls, failed_set, web_name):
    """
    ...
    if pd.isna(image_urls) or str(image_urls).strip() == "":
        return ""
    new_image_urls = []
    for url in str(image_urls).split(images_split):
        url = url.strip()
