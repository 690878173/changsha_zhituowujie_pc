    ...
    def extract_image_urls(self):
        ...
        for value in df["Images"].dropna():
            if not isinstance(value, str):
                value = str(value)
            for url in value.split(self.config["REQUEST"].get("csv_images_split", ",")):
                url = url.strip()
        ...
        for attempt in range(1, self.max_attempts + 1):
            ...
            try:
                ...
                if len(image_url) >= 1000:
                    safe_print(f'[WA] URI Too Large,url before 50 size:{image_url[:51]}')
                ...
                safe_print(f"Attempt {attempt} failed: HTTP {status_code} - {exc}, url={image_url[:60]}...")
