...
class Pc(Get_Product):
    ...
    def zdy_zd(self,url):
        ...
        def _f(name,value):
            dic[name] = Tool.HTML.clean_product_desc(value[0])

        fet = html.xpath('//details[@class="main-product__tab-details"]')
        for l in fet:
            name = l.xpath('./summary//text()')
            name = ''.join(name)

            for i in ['Product Details','Material & Care']:
                if i in name:
                    value = l.xpath('./div')
                    _f(i,value)

        # if fet:
        #     _f('Product Details',fet)
        #
        #
        #
        #
        #
        # ls = html.xpath('//div[@class="main-product__info-blocks is-anim-init"]/div')
        # print(ls)
        # for l in ls:
        #     _name = l.get('class')
        #     print(_name)
        #     for i in ['benefits','features','materials']:
        #         if i in _name:
        #             value = l.xpath('./div/details')[0]
