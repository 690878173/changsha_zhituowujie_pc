...
def f1(url_dic):
    ...
    ml1 = html.xpath('//ul[@class="header__nav-list"]/li')
    header_node = ml1


    for node in header_node:

        c_node = node.xpath('./div/nav')[0]
        _name = c_node.xpath('./h2/text()')[0].strip()
        _name = ''.join(_name).replace('"','').strip()
        _url = url
        _url = Tool.URL.add_site(_url)

        url_dic[_name] = {'child':{}}

        childs = c_node.xpath('./div/div')
...
def f2(dic, childs):
    for child in childs:
        ...
        _name = child.xpath('./h3/text()')

        if not _name:
            _name = c_node[0].xpath('./span/text()')
            c_node = c_node[1:]
