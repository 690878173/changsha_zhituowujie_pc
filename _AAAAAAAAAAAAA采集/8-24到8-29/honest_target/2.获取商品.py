...
if __name__ == "__main__":
    crawler = TargetDrissionCrawler(
        input_file=r"data/detail_url_the-honest-company.json",
        output_file=r"data/finally.csv",
        fail_file=r"data/fail1.json",
        proxy="127.0.0.1:7897",
        headless=False,
        num_threads=3,         # 3个标签页并行处理（建议2-4，太高可能触发反爬）
        max_wait_price=12,     # 智能等待价格元素最多12秒（通常3-5秒就好）
    )
    crawler.run()
