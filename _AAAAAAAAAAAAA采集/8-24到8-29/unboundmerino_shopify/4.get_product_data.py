...
class Pc(Get_Product):
    ...
    def zdy_zd(self,url):
        ...
        dic = {}
        ls = html.xpath('//div[@class="product-tabs"]/div')

        for l in ls:
            name = l.xpath('./div[@class="product-tab__heading"]//text()')
            name = ''.join(name).strip()

            for i in ['The Fit','Fabric and Details']:
                if i in name:
                    dic[i] = Tool.HTML.clean_product_desc(l.xpath('./div[@class="product-tab__content-container"]')[0])
        # fit = html.xpath('//div[@class="product-tab tab-the-fit"]')
        # detail = html.xpath('//div[@class="product-tab tab-the-details product-tab--woolmark-eligible"]')
        # if fit:
        #     dic['The Fit'] = Tool.HTML.clean_product_desc(fit[0])
        # if detail:
        #     dic['Details'] = Tool.HTML.clean_product_desc(detail[0])
        return dic
