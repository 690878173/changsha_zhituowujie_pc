...
def f1(url_dic):
    ...
    ml1 = html.xpath('//div[@class="megamenu__container"]')[:2]
    header_node = ml1
    ...
    for node in header_node:
        ...
        _name = 'Men' if num else 'Women'
        num -= 1

        url_dic[_name] = {'child':{}}

        childs = node.xpath('.//div[@class="megamenu__list"]')
...
def f2(dic, childs):
    for child in childs:
        node_list = child.xpath('./ul/li')

        c_node = child.xpath('./div[@class="megamenu__secondary-header"]/a')[0]
