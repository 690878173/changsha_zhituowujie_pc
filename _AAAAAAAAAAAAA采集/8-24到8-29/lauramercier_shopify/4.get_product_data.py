...
class Pc(Get_Product):
    ...
    def zdy_zd(self,url):
        ...
        ls = html.xpath('//div[@class="product-block-accordion tw-group tw-w-full "]')

        for l in ls:
            name = l.xpath('./label/span//text()')
            name = ''.join(name).strip()

            for i in ['Details','Usage','Ingredients']:
                if i in name:
                    dic[i] = Tool.HTML.clean_product_desc(l)
        # detail = html.xpath('//div[@class="description-accordion"]/div')
        # if detail:
        #     dic['Detail'] = Tool.HTML.clean_product_desc(detail[0])
